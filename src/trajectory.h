...
vector<vector<double>> getTrajectory(vector<double> start, double target_speed, double max_jerk, double max_acceleration){
  ...
  if (fabs(start[2]) >= (max_jerk/50.0) && fabs(target_speed - start[1]) <= 0.5 * pow(start[2], 2)/max_jerk){
    double t = fabs(start[2])/max_jerk;
    double a = 0;
  ...
  double t_a = -start[2] / (((target_speed > start[1])? 1.0:-1.0)*max_jerk);
  ...
  double t_r = std::min(sqrt(fabs(target_speed - v_star)/ max_jerk), max_acceleration/max_jerk);
  ...
  double t_s = fabs(target_speed - v3)/ max_acceleration;
  if (t_r + t_a > 0){
  ...
  if (trajectory.size()<50 && fabs(target_speed - start[1])<0.5 && fabs(start[2]) < (max_jerk/50.0)){
    for (int i=1; i<17; i++){
